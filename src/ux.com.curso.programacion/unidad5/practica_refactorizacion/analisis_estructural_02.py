...
import statistics  # SE AGREGA: Para el cálculo eficiente y profesional de la mediana
...
def limpiar_nombre_usuario_refactorizada(nombre_sucio):
    # CAMBIO: Se eliminan bucles manuales y operaciones de conversión ASCII.
    # POR QUÉ: '.strip()' elimina espacios en los extremos eficientemente y '.capitalize()' 
    # asegura la primera mayúscula y el resto minúsculas de forma nativa.
...
def contiene_palabra_bloqueada_refactorizada(mensaje_chat, palabra_prohibida):
    # CAMBIO: Se elimina el doble ciclo indexado complejo.
    # POR QUÉ: El operador nativo 'in' de Python implementa algoritmos de búsqueda 
    # optimizados en C de manera directa, limpia y legible.
...
def generar_clave_temporal_refactorizada():
    caracteres_validos = "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnpqrstuvwxyz23456789"
    # CAMBIO: Se elimina el bucle 'for' y la concatenación destructiva de strings.
    # POR QUÉ: 'random.choices' extrae múltiples elementos en un solo paso, e 
    # ''.join()' los une eficientemente sin recrear variables en memoria constantemente.
...
def calcular_mediana_latencia_refactorizada(lista_pings):
    # CAMBIO: Se desecha por completo el ordenamiento por Burbuja manual y el cálculo de la mediana.
    # POR QUÉ: La librería estándar de Python tiene el módulo 'statistics'. El método 
    # 'statistics.median()' se encarga de clonar, ordenar de manera eficiente O(n log n) 
    # y calcular el centro exacto de forma directa sin importar si la lista es par o impar.
    return statistics.median(lista_pings)


# === PROGRAMA PRINCIPAL (Punto de entrada modificado) ===
if __name__ == "__main__":
    ...
    print("VERSIONES REFACTORIZADAS")
    
    print("Usuario limpio (Refactorizado):", [limpiar_nombre_usuario_refactorizada("   luNA_eDUaRDo  ")])
    print("¿Tiene groserías? (Refactorizado):", contiene_palabra_bloqueada_refactorizada(msg, "malas"))
    print("Clave generada (Refactorizada):", generar_clave_temporal_refactorizada())
    print("Mediana de latencia (Refactorizada):", calcular_mediana_latencia_refactorizada(pings_servidor))